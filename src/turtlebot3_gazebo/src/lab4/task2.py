...
class Task2(Node):
    ...
    def __laser_cbk(self, msg):
        ...
        if (min(msg.ranges[0:60]+msg.ranges[300:360])<0.25):
    ...
    def PID_angular(self, angular_error):
        kp_ang, kd_ang, ki_ang, dt = 35, 35.5, 0.001, 0.1
    ...
    def publish_path(self, poses):
        ...
        for pose in path_arr.poses:
            pose.pose.position.x = 1.6 * (pose.pose.position.x) + offset_x
    ...
    def run(self):
        while rclpy.ok():
            ...
            if (self.goal_pose.pose.position.x != self.ttbot_pose.pose.pose.position.x or self.goal_pose.pose.position.y != self.ttbot_pose.pose.pose.position.y):
                ...
                node_path, path = self.a_star_path_planner(self.ttbot_pose.pose, self.goal_pose)
